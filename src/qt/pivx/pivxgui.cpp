...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("sagiso client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("sagiso"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("sagiso"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
