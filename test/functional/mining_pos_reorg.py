...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected sagiso and zsagiso supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zsagisosupply']:
                assert_equal(node['zsagisosupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check sagiso and zsagiso supply at the beginning
        # ------------------------------------------
        # zsagiso supply: 2 coins for each denomination
        ...
        # sagiso supply: block rewards minus burned fees for minting
        ...
        # Verify that sagiso and zsagiso supplies were properly updated after the spends and reorgs
        self.log.info("Check sagiso and zsagiso supply...")
